...
Future<List<String>> r6ValidationXml() async {
  var dir = Directory('./test/r6_xml_examples');
  var string = <String>[];
  for (var file in await dir.list().toList()) {
    // print(file.path);
    ...
    r6.Resource? xmlResource;

    /// try because if the formatting is off then it will throw an error.
    try {
      xmlResource = r6.Resource.fromXml(xmlContents);
      ...
      final jsonContents =
          (await File('./test/r6_examples/$jsonFileName').readAsString())
              .replaceAll('4.3.0-cibuild', '4.3.0');
      ...
      xmlResource = r6.Resource.fromJson(xmlMap);
      var jsonResource = r6.Resource.fromJson(jsonMap);
